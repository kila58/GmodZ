...
function ENT:Attack()
	...
	dmginfo:SetDamage( math.Rand( 15, 25 ))
	...
	timer.Simple( 0.3, function()
		...
		if self and self:IsValid() then
			...
			for _,v in pairs( ents.FindInSphere( (self:GetPos() + Vector( 0, 0, 50)) + self:GetForward() * 20, 40 )) do
				if v:IsPlayer() then
					hit = true
			...
			if hit then
				self:EmitSound( "npc/fast_zombie/claw_strike" .. math.Round( math.Rand( 1, 3 )) .. ".wav", 70, 100 );
			else
				self:EmitSound( "npc/fast_zombie/claw_miss" .. math.Round( math.Rand( 1, 2 )) .. ".wav", 70, 100 );
			end
...
function ENT:Think()
	...
	if ( self.Dead or self:IsCurrentSchedule( SCHED_MELEE_ATTACK1 ) 
		...
		or self:IsCurrentSchedule( SCHED_FORCED_GO_RUN ) ) then
		
			if self.NextShout < CurTime() then
				self.NextShout = CurTime() + 30
				self:EmitSound( "npc/fast_zombie/fz_alert_close1.wav", 85, math.random(45, 55) )
			end
